...
<head>
    ...
    <link rel="apple-touch-icon" type="image/x-icon" href="/cyw/80hou/favicon.ico">
    <link rel="shortcut icon" type="image/x-icon" href="/cyw/80hou/logo.png">
    <link rel="stylesheet" type="text/css" href="/cyw/80hou/Public/libs/lyui/dist/css/lyui.min.css">
    <link rel="stylesheet" type="text/css" href="/cyw/80hou/./Application/Admin/View/Public/css/<?php echo C('ADMIN_THEME');?>.css">
    
    <link rel="stylesheet" type="text/css" href="/cyw/80hou/Public/libs/lyui/dist/css/lyui.extend.min.css">
    <style type="text/css">
    /* Builder样式 */
    ...
    <![endif]-->
    <script type="text/javascript" src="/cyw/80hou/Public/libs/jquery/1.x/jquery.min.js"></script>
</head>
<body class="<?php echo ($_page_name); ?>">
                        ...
                        <div>
                            <div class="navbar-header navbar-header-default">
                                ...
                                <?php if(C('WEB_SITE_LOGO')): ?>
                                    <a class="navbar-brand" target="_blank" href="/cyw/80hou/">
                            ...
                            <div class="collapse navbar-collapse navbar-collapse-top">
                                ...
                                <ul class="nav navbar-nav navbar-right">
                                    ...
                                    <li><a target="_blank" href="/cyw/80hou/"><i class="fa fa-external-link"></i> 打开前台</a></li>
                                    <li class="dropdown">
                                        <a href="#" class="dropdown-toggle" data-toggle="dropdown">
    ...
    <div class="col-xs-2">
        <link rel="stylesheet" type="text/css" href="/cyw/80hou/Public/libs/ztree/css/metroStyle/metroStyle.css">
<div class="panel-body">
    <div class="ztree ztree-manual" id="cate-list"></div>
</div>
<?php
 $map['status'] = array('egt', '0'); $map['group'] = array('eq', 1); $data_list = D('Category')->field('id,pid,group,doc_type,title,url,create_time,sort,status') ->where($map)->order('sort asc,id asc')->select(); foreach ($data_list as &$item) { $document_type = D('Type')->find($item['doc_type']); if ($document_type['name'] === 'page') { $item['url'] = U('Cms/Category/edit_with_tree', array('id' => $item['id'])); $item['target'] = "_self"; } else if ($document_type['name'] === 'link') { if (!stristr($item['url'], 'http://') && !stristr($item['url'], 'https://')) { $item['url'] = U($item['url']); } } else if ($document_type['system'] === '0') { $item['url'] = U('Cms/Index/index', array('cid' => $item['id'])); $item['target'] = "_self"; } } $data_list = json_encode($data_list); ?>
<script type="text/javascript" src="/cyw/80hou/Public/libs/ztree/js/jquery.ztree.all-3.5.min.js" charset="utf-8"></script>
...
<?php else: ?>
    <?php echo(date("Y-m-d H:i:s")); endif; ?>" <?php echo ((isset($form["extra"]["attr"]) && ($form["extra"]["attr"] !== ""))?($form["extra"]["attr"]):''); ?>>
            ...
            <script type="text/javascript">
                $(function(){
                    var uploader_<?php echo ((isset($group_k) && ($group_k !== ""))?($group_k):''); ?>_upload_<?php echo ($k); ?> = WebUploader.create({
                        ...
                        swf: '/cyw/80hou/Public/libs/lyui/dist/swf/uploader.swf',                                                  // swf文件路径
            ...
            <script type="text/javascript">
                $(function(){
                    var uploader_<?php echo ((isset($group_k) && ($group_k !== ""))?($group_k):''); ?>_upload_<?php echo ($k); ?> = WebUploader.create({
                        ...
                        fileSingleSizeLimit:<?php echo isset($form['extra']['self']['size']) ? $form['extra']['self']['size'] : C('UPLOAD_IMAGE_SIZE'); ?>*1024*1024, // 验证单个文件大小是否超出限制, 超出则不允许加入队列
                        // 文件过滤
                        accept: {
                            title: 'Images',
                            extensions: "<?php echo isset($form['extra']['self']['ext']) ? $form['extra']['self']['ext'] : 'gif,jpg,jpeg,bmp,png'; ?>",
                            mimeTypes: 'image/gif,image/jpg,image/jpeg,image/bmp,image/png'
                        }
                    });
        ...
        <div class="right">
            ...
            <script type="text/javascript" src="/cyw/80hou/Public/libs/kindeditor/kindeditor-min.js" charset="utf-8"></script>
            <script type="text/javascript">
                $(function(){
                    KindEditor.ready(function(K) {
                        kindeditor_<?php echo ($k); ?> = K.create('#<?php echo ((isset($group_k) && ($group_k !== ""))?($group_k):''); ?>_kindeditor_<?php echo ($k); ?>', {
                            ...
                            filePostName : 'file',
                            cssPath : [
                                '/cyw/80hou/Public/libs/lyui/dist/css/lyui.min.css',
                                '/cyw/80hou/Public/libs/kindeditor/plugins/code/prettify.css'
                            ],
                            width : '100%',
                            height : '500px',
                            resizeType: 1,
        ...
        <div class="right">
            ...
            <link rel="stylesheet" type="text/css" href="/cyw/80hou/Public/libs/jquery_tokeninput/css/token-input.css">
            <link rel="stylesheet" type="text/css" href="/cyw/80hou/Public/libs/jquery_tokeninput/css/token-input-bootstrap.css">
            <script type="text/javascript" src="/cyw/80hou/Public/libs/jquery_tokeninput/js/jquery.tokeninput.min.js" charset="utf-8"></script>
            <script type="text/javascript">
                $(function(){
                    //标签自动完成
                    ...
                    tags.tokenInput("<?php echo U(MODULE_NAME.'/Tag/search'); ?>",{
                        ...
                        onAdd: function (item){ //新增系统没有的标签时提交到数据库
                            $.post("<?php echo U(MODULE_NAME.'/Tag/add'); ?>", {'title': item.title});
                                ...
                                <?php case "date_price": ?><div class="form-group item_<?php echo ($form["name"]); ?> <?php echo ((isset($form["extra_class"]) && ($form["extra_class"] !== ""))?($form["extra_class"]):''); ?>">
	<label class="left control-label">节日价格：</label>
	<div class="right">
        <div class="right">
			<div id="goods_spec_table2">
				<table class="table table-bordered">
					<tbody id="date_price_tab">
						<tr>
							<td><b>日期</b></td>
							<td><b>价格</b></td>
							<td><b>操作
						...
						<?php if(!empty($date_price)): if(is_array($date_price)): $i = 0; $__LIST__ = $date_price;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$date_item): $mod = ($i % 2 );++$i;?><tr>
								<td>
									<input name="date_price[date][]" type="text" class="form-control input date price_date" value="<?php echo $date_item['date'];?>">
								</td>
								<td>
									<input name="date_price[price][]" style="padding: 5px 12px;" value="<?php echo ($date_item['price']); ?>">
								</td>
								<td>
									<a name="delete" title="删除" class="label label-danger-outline label-pill date_price_del"  href="javascript:;">删除</a>
								</td>
							</tr><?php endforeach; endif; else: echo "" ;endif; endif; ?>
					</tbody>
				</table>
				<button type="button" class="btn btn-default add_date_price">添加节日价格</button>
			</div>
		</div>
    </div>
	<script>
	$(function(){
		$(document).on('click', '.date_price_del', function(){
			$(this).parents('tr').remove();
		});
		$(".add_date_price").on('click', function(){
			html = '<tr>'+
						'<td>'+
							'<input name="date_price[date][]" type="text" class="form-control input date price_date" value="">'+
						'</td>'+
						'<td>'+
							'<input name="date_price[price][]" style="padding: 5px 12px;" value="">'+
						'</td>'+
						'<td>'+
							'<a name="delete" title="删除" class="label label-danger-outline label-pill date_price_del"  href="javascript:;">删除</a>'+
						'</td>'+
					'</tr>';
			$("#date_price_tab").append(html);
			price_date_pick();
		});
		price_date_pick();
	});
	function price_date_pick() {
		$(".price_date").each(function(index, el) {
			$(el).datetimepicker({
		        format      : 'yyyy-mm-dd',
		        autoclose   : true,
		        minView     : 'month',
		        ...
		        fontAwesome : true
                                ...
                                <?php case "address": ?><div class="form-group item_<?php echo ($form["name"]); ?> <?php echo ((isset($form["extra"]["class"]) && ($form["extra"]["class"] !== ""))?($form["extra"]["class"]):''); ?>">
        <label class="left control-label"><?php echo ($form["title"]); ?>：</label>
        <div class="right">
            <select name="prov" class="form-control cui-select select area" style="width:140px;">
                <option value=''>请选择：</option>
                <?php if(is_array($province_list)): foreach($province_list as $option_key=>$option): ?><option value="<?php echo ($option_key); ?>" <?php if(($prov) == $option_key): ?>selected<?php endif; ?>><?php echo ($option); ?></option><?php endforeach; endif; ?>
            </select>
            <select name="city" class="form-control cui-select select area"  style="width:140px;">
                <option value=''>请选择：</option>
                <?php if(is_array($city_list)): foreach($city_list as $option_key=>$option): ?><option value="<?php echo ($option_key); ?>" <?php if(($city) == $option_key): ?>selected<?php endif; ?>><?php echo ($option); ?></option><?php endforeach; endif; ?>
            </select>
            <select name="country" class="form-control cui-select select area"  style="width:140px;">
                <option value=''>请选择：</option>
                <?php if(is_array($district_list)): foreach($district_list as $option_key=>$option): ?><option value="<?php echo ($option_key); ?>" <?php if(($country) == $option_key): ?>selected<?php endif; ?>><?php echo ($option); ?></option><?php endforeach; endif; ?>
            </select>
            <input type="text" class="form-control input text" name="address" value="<?php echo ((isset($address) && ($address !== ""))?($address):''); ?>"  style="width:290px;display: inline;">
        </div>
    </div>
    <script>
        $(function(){
        ...
        <div class="right">
            ...
            <link rel="stylesheet" href="/cyw/80hou/Public/libs/jquery-asDatepicker/css/asDatepicker.css">
            <link rel="stylesheet" href="/cyw/80hou/Public/libs/jquery-asDatepicker/css/corethink.css">
            <script type="text/javascript" src="/cyw/80hou/Public/libs/jquery-asDatepicker/dist/jquery-asDatepicker.js"></script>
            <script type="text/javascript">
                $(function(){
             		$('[name=<?php echo ($form["name"]); ?>]').asDatepicker({
                        ...
                        default_value: "<?php echo ($form["value"]); ?>",
             			onInit:function(){
             				this.$inputIcon.remove();
                            // this._date.selectedDate='1482422400000,1482508800000';
            ...
            <?php if(!empty($form["tip"])): ?><span class="check-tips small"><?php echo ($form["tip"]); ?></span><?php endif; ?>
                                ...
                                <?php case "order_house": ?><div class="form-group item_<?php echo ($form["name"]); ?> <?php echo ((isset($form["extra_class"]) && ($form["extra_class"] !== ""))?($form["extra_class"]):''); ?>">
	<label class="left control-label"><?php echo ($form["title"]); ?>：</label>
        <div class="right">
			<?php if(!empty($house_list)): ?><p>房源id：<?php echo ($house_list[0]['house_id']); ?></p>
	        	<p>房间号：<?php echo ($house_list[0]['room_number']); ?></p>
	        	<p>房源标题：<?php echo ($house_list[0]['title']); ?></p>
	        	<p>目的地：<?php echo ($house_list[0]['destination_name']); ?></p>
				<table class="table table-bordered" id="spec_input_tab">
					<tbody>
						...
						<?php if(is_array($house_list)): $i = 0; $__LIST__ = $house_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$house): $mod = ($i % 2 );++$i;?><tr>
							<td><?php echo ($house['date']); ?></td>
							<td><?php echo ($house['house_price']); ?></td>
                                ...
                                <?php case "order_custom": ?><div class="form-group item_<?php echo ($form["name"]); ?> <?php echo ((isset($form["extra_class"]) && ($form["extra_class"] !== ""))?($form["extra_class"]):''); ?>">
	<label class="left control-label"><?php echo ($form["title"]); ?>：</label>
	<div class="right">
        <div class="right">
			<?php if(!empty($custom_list)): ?><table class="table table-bordered" id="spec_input_tab">
					<tbody>
						<tr>
							<td><b>目的地</b></td>
							<td><b>成人人数</b></td>
						...
						<?php if(is_array($custom_list)): $i = 0; $__LIST__ = $custom_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$custom): $mod = ($i % 2 );++$i;?><tr>
							<td><?php echo ($custom['destination_name']); ?></td>
							<td><?php echo ($custom['adults_number']); ?></td>
							<td><?php echo ($custom['children_number']); ?></td>
							<td><?php echo ($custom['departure_time']); ?></td>
						</tr><?php endforeach; endif; else: echo "" ;endif; ?>
					</tbody>
				</table><?php endif; ?>
		</div>
            ...
            <script type="text/javascript">
                $(function(){
                    var uploader_tab_<?php echo ((isset($group_k) && ($group_k !== ""))?($group_k):''); ?>_upload_<?php echo ($tab_k); ?> = WebUploader.create({
                        ...
                        resize: false,                                                                     // 不压缩image, 默认如果是jpeg，文件上传前会压缩一把再上传！
                        fileNumLimit: 20,                                                                  // 验证文件总数量, 超出则不允许加入队列
                        fileSingleSizeLimit:<?php echo isset($tab_form['extra']['self']['size']) ? $tab_form['extra']['self']['size'] : C('UPLOAD_IMAGE_SIZE'); ?>*1024*1024, // 验证单个文件大小是否超出限制, 超出则不允许加入队列
                        // 文件过滤
                        accept: {
                            title: 'Images',
                            extensions: "<?php echo isset($tab_form['extra']['self']['ext']) ? $tab_form['extra']['self']['ext'] : 'gif,jpg,jpeg,bmp,png'; ?>",
                            mimeTypes: 'image/gif,image/jpg,image/jpeg,image/bmp,image/png'
                        }
                    });
...
<?php else: ?>
    ...
    <div class="clearfix full-script">
        <div class="container-fluid">
            <input type="hidden" id="corethink_home_img" value="/cyw/80hou/./Application/Home/View/Public/img">
            <script type="text/javascript" src="/cyw/80hou/Public/libs/lyui/dist/js/lyui.min.js"></script>
            <script type="text/javascript" src="/cyw/80hou/./Application/Admin/View/Public/js/admin.js"></script>
            <script type="text/javascript">
                // 如果是多标签方式自动跳转后台首页
    ...
    <script type="text/javascript" src="/cyw/80hou/Public/libs/lyui/dist/js/lyui.extend.min.js"></script>
    <script type="text/javascript">
    $(function() {
        //给数组增加查找指定的元素索引方法
