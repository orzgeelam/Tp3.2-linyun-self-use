...
class ModelConfigEditorController extends AddonController
{
    ...
    {
        if (IS_POST) {
            $model = D('Addons://ModelConfigEditor/Module');
            if ($data = $model->create()) {
                $res = $model->build($data);
                if ($res) {
                    $this->success('生成成功');
                } else {
                    $model->build_fail($data['name']);
                    $this->error('生成失败:' . $model->getError());
                }
            } else {
            ...
            $builder->setMetaTitle('一键生成模块demo') // 设置页面标题
                ->addFormItem('name', 'text', '模块名称', '首字母大写的一串连续字母')
                ->addFormItem('title', 'text', '模块标题', '2-4个汉字')
                ->addFormItem('icon', 'icon', '模块图标', '图标')
                ->setSubmitTitle('一键生成')
                ->display();
        }
    }
