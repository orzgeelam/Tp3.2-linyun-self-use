...
class AdminController extends ControllerController
{
	 ...
	 * 获取后台用户数据
	 */
	public function lists($tableName, $map = '1', $order = 'id DESC')
	{
		// 获取列表
		$p            = I('get.p', 1);
	...
	public function module_config()
	{
		if (IS_POST) {
			...
			$module_info   = D('Admin/Module')->where(['name' => $name])->find();
