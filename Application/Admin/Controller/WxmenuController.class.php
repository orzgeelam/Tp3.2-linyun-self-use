...
class WxmenuController extends AdminController
{
	...
	{
		$datalist = D('imgtext')->select();//获取数据
		// 使用Builder快速建立列表页面。
		$builder = new \Common\Builder\ListBuilder();
		$builder->setMetaTitle("列表")// 设置页面标题
		        ->addTopButton("addnew", ['href' => U('addit')])// 添加新增按钮
		        ->addTableColumn("id", "id")
		        ->addTableColumn("pics", "图片")
		        ->addTableColumn("title", "标题")
		        ->addTableColumn("createdate", "创建日期")
		        ->addTableColumn("url", "内容")
		        ->addTableColumn("right_button", "操作管理", "btn")
	...
	{
		if (IS_POST) {
			if (D('imgtext')->add(I('post.'))) {
				$this->success('添加成功');
			} else {
				trace(D('imgtext')->getError());
				$this->error('添加失败');
			}
		} else {
			// 使用FormBuilder快速建立表单页面
			$builder = new \Common\Builder\FormBuilder();
			$builder->setMetaTitle('新增图文')// 设置页面标题
			        ->setPostUrl(U('addit'))// 设置表单提交地址
			        ->addFormItem('pics', 'picture', '上传图片')
			        ->addFormItem('title', 'text', '图文标题')
			        ->addFormItem('describe', 'text', '图文描述', '')
			        ->addFormItem('url', 'text', '外部链接', '')
				// ->addFormItem('sort', 'num', '排序', '')
				// ->setFormData([
				//     'sort' => D('imgtext')->MAX('sort') + 1,
				//     'fid'  => $fid,
				// ])
				// ->setExtraHtml('<script>$("[name=fid] option:contains(\'请选择\')").val(0);</script>')
				    ->display();
