	...
	public function install()
	{
		...
		if ($hooks) {
			...
			foreach ($hooks as $val) {
				$hook_object->existHook($val, ['description' => $info['description']]);
			}
		}
		// 安装数据库
	...
	public function adminList($name, $tab = 1)
	{
		...
		if ($addon->custom_adminlist) {
			...
			foreach ($admin_list as $key => $val) {
				...
				$tab_list[$key]['href']  = U('Admin/Addon/adminList', [
					'name' => $name,
					'tab'  => $key,
				]);
			}
			...
			if ($param) {
				...
				$builder->setMetaTitle($addon->info['title'])// 设置页面标题
				        ->AddTopButton('addnew', ['href' => U('Admin/Addon/adminAdd', ['name' => $name, 'tab' => $tab])])// 添加新增按钮
				        ->AddTopButton('resume', ['model' => $param['model']])// 添加启用按钮
				        ->AddTopButton('forbid', ['model' => $param['model']])// 添加禁用按钮
				        ->setSearch('请输入关键字', U('Admin/Addon/adminList', ['name' => $name, 'tab' => $tab]))
				...
				$attr['href']  = U('Admin/Addon/adminEdit', [
					'name' => $name,
					'tab'  => $tab,
					'id'   => '__data_id__',
				]);
				...
				$builder->addTableColumn('right_button', '操作', 'btn')
				        ...
				        ->addRightButton('forbid', ['model' => $param['model']])// 添加禁用/启用按钮
				        ->addRightButton('delete', ['model' => $param['model']])// 添加删除按钮
				        ->display();
			} else {
