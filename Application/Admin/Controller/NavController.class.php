...
class NavController extends AdminController
{
	...
	public function index($group = 'main')
	{
		...
		foreach ($data_list as &$val) {
			if ($val['type'] == 'post') {
				$val['title'] = '<a href="'.U('Admin/Post/index', ['cid' => $val['id']]).'">'.$val['title'].'</a>';
			}
		}
		// 转换成树状列表
		$tree      = new Tree();
		...
		foreach ($nav_group_list as $key => $val) {
			...
			$tab_list[$key]['href']  = U('index', ['group' => $key]);
		}
		// 使用Builder快速建立列表页面。
		$builder = new \Common\Builder\ListBuilder();
		$builder->setMetaTitle('导航列表')// 设置页面标题
		        ->addTopButton('addnew', ['href' => U('Admin/Nav/add', ['group' => $group])])// 添加新增按钮
		        ->addTopButton('resume')// 添加启用按钮
		        ->addTopButton('forbid')// 添加禁用按钮
		        ->addTopButton('delete')// 添加删除按钮
		        ->setSearch('请输入ID/导航名称', U('index', ['group' => $group]))
		        ...
		        ->addRightButton('edit', ['href' => U('edit', ['group' => $group, 'id' => '__data_id__'])])// 添加编辑按钮
	...
	public function add($group)
	{
		if (IS_POST) {
			...
			$builder->setMetaTitle('新增导航')// 设置页面标题
			        ->setPostUrl(U('', ['group' => $group]))// 设置表单提交地址
			        ->addFormItem('group', 'hidden', '导航分组', '导航分组')
			        ->addFormItem('pid', 'select', '上级导航', '上级导航', select_list_as_tree('Admin/Nav', ['group' => $group], '顶级导航'))
			        ...
			        ->addFormItem('target', 'radio', '打开方式', '打开方式', ['' => '当前窗口', '_blank' => '新窗口打开'])
			        ->addFormItem('icon', 'icon', '图标', '导航图标')
			        ->addFormItem('sort', 'num', '排序', '用于显示的顺序')
			        ->setFormData(['type' => 'link', 'group' => $group])
