...
class AddonModel extends ModelModel
{
	...
	protected $_validate
		= [
			['name', 'require', '插件名称不能为空', self::EXISTS_VALIDATE, 'regex', self::MODEL_BOTH],
			['name', '1,32', '插件名称长度为1-32个字符', self::EXISTS_VALIDATE, 'length', self::MODEL_BOTH],
			['name', '', '插件名称已经存在', self::VALUE_VALIDATE, 'unique', self::MODEL_BOTH],
			['description', 'require', '钩子描述必须！', self::EXISTS_VALIDATE, 'regex', self::MODEL_BOTH],
		];
	/**
	 * 自动完成规则
	 */
		...
		foreach ($addons as &$val) {
			switch ($val['status']) {
				...
				case '0':  // 禁用
					...
					$val['right_button']['uninstall']['attribute'] = 'class="label label-danger ajax-get" href="'.U('uninstall', ['id' => $val['id']]).'"';
					if ($val['adminlist']) {
						$val['right_button']['adminlist']['title']     = '数据管理';
						$val['right_button']['adminlist']['attribute'] = 'class="label label-success" href="'.U('adminlist', ['name' => $val['name']]).'"';
					}
					break;
				case '1':  // 正常
	 ...
	 */
	public function getAddonUrl($url, $param = [])
	{
		$url        = parse_url($url);
