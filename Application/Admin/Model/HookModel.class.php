...
class HookModel extends ModelModel
{
	...
	protected $_validate
		= [
			['name', 'require', '钩子名称必须！', self::MUST_VALIDATE, 'regex', self::MODEL_BOTH],
			['name', '1,32', '钩子名称长度为1-32个字符', self::EXISTS_VALIDATE, 'length', self::MODEL_BOTH],
			['name', '', '钩子名称已经存在', self::VALUE_VALIDATE, 'unique', self::MODEL_BOTH],
			['description', 'require', '钩子描述必须！', self::MUST_VALIDATE, 'regex', self::MODEL_BOTH],
		];
	/**
	 * 自动完成规则
	 */
	...
	public function updateHooks($addons_name)
	{
		...
		if (!empty($common)) {
			foreach ($common as $hook) {
				$flag = $this->updateAddons($hook, [$addons_name]);
				if (false === $flag) {
	...
	public function removeHooks($addons_name)
	{
		...
		if ($common) {
			foreach ($common as $hook) {
				$flag = $this->removeAddons($hook, [$addons_name]);
				if (false === $flag) {
					return false;
				}
			}
