...
class GroupModel extends ModelModel
{
	...
	protected $_validate
		= [
			['title', 'require', '部门名称不能为空', self::MUST_VALIDATE, 'regex', self::MODEL_BOTH],
			['title', '1,32', '部门名称长度为1-32个字符', self::EXISTS_VALIDATE, 'length', self::MODEL_BOTH],
			['title', '', '部门名称已经存在', self::VALUE_VALIDATE, 'unique', self::MODEL_BOTH],
			['menu_auth', 'require', '权限不能为空', self::MUST_VALIDATE, 'regex', self::MODEL_BOTH],
		];
	/**
	 * 自动完成规则
	 */
