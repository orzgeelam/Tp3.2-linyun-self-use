...
class NavModel extends ModelModel
{
	...
	protected $_validate
		= [
			['group', 'require', '导航分组必须', self::MUST_VALIDATE, 'regex', self::MODEL_INSERT],
			['title', 'require', '导航标题不能为空', self::MUST_VALIDATE, 'regex', self::MODEL_BOTH],
			['type', 'require', '导航类型不能为空', self::MUST_VALIDATE, 'regex', self::MODEL_BOTH],
			['url', '0,255', '链接长度为0-25个字符', self::EXISTS_VALIDATE, 'length', self::MODEL_BOTH],
		];
	/**
	 * 自动完成规则
	 */
	protected $_auto
		= [
			['value', 'get_value_by_type', self::MODEL_BOTH, 'callback'],
	...
	protected function _after_find(&$result, $options)
	{
		...
		switch ($result['type']) {
			...
			case 'page':
				...
				$result['href']    = U('Home/Nav/page', ['id' => $result['id']], true, true);
				break;
			case 'post':
				$result['href'] = U('Home/Nav/lists', ['id' => $result['id']], true, true);
				break;
		}
	}
	...
	{
		if (!in_array(MODULE_NAME, ['Home', 'Admin', 'Common', 'Install']) && is_file(APP_PATH.MODULE_NAME.'/Model/NavModel.class.php')) {
