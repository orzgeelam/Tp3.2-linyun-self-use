...
class UploadModel extends ModelModel
{
	...
	protected $_validate
		= [
			['name', 'require', '文件名不能为空', self::EXISTS_VALIDATE, 'regex', self::MODEL_BOTH],
			['path', 'require', '文件不能为空', self::EXISTS_VALIDATE, 'regex', self::MODEL_BOTH],
			['size', 'require', '文件大小不能为空', self::EXISTS_VALIDATE, 'regex', self::MODEL_BOTH],
			['md5', 'require', '文件Md5编码不能为空', self::EXISTS_VALIDATE, 'regex', self::MODEL_BOTH],
			['sha1', 'require', '文件Sha1编码不能为空', self::EXISTS_VALIDATE, 'regex', self::MODEL_BOTH],
		];
	/**
	 * 自动完成规则
	 */
	protected $_auto
		= [
			['uid', 'is_login', self::MODEL_INSERT, 'function'],
	...
	protected function _after_find(&$result, $options)
	{
		...
		if (in_array($result['ext'], ['jpg', 'jpeg', 'png', 'gif', 'bmp'])) {
	...
	{
		// 上传token
		$upload_token = \Common\Util\Think\Strings::randString(32, 1);
		S('upload_token', $upload_token, ['prefix' => ENV_PRE.'Home_', 'expire' => 300]);
		// curl
		...
		// ptrace($post_data);
		$ch = curl_init();
		curl_setopt($ch, CURLOPT_HTTPHEADER, ['uploadtoken: '.$upload_token]);
		...
		// ob_end_flush();
		$result = curl_exec($ch);
		// ptrace(curl_errno($ch));
		// ptrace(curl_error($ch));
		// ptrace($result);
		curl_close($ch);
		return $result;
	}
	...
	public function upload($files)
	{
		...
		if ($upload) {
			...
			// 上传文件
			$upload_config['removeTrash'] = [$this, 'removeTrash'];
	...
	private function downLocalFile($file, $callback = null, $args = null)
	{
		...
		if (file_exists($fiel_path)) {
			...
			// 新增下载数
			$this->where(['id' => $file['id']])->setInc('download');
			// 执行下载
