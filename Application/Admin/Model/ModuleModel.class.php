...
class ModuleModel extends ModelModel
{
	...
	protected $_validate
		= [
			['name', 'require', '模块名称不能为空', self::MUST_VALIDATE, 'regex', self::MODEL_BOTH],
			['name', '', '该模块已存在', self::MUST_VALIDATE, 'unique', self::MODEL_BOTH],
			['title', 'require', '模块标题不能为空', self::MUST_VALIDATE, 'regex', self::MODEL_BOTH],
			['description', 'require', '模块描述不能为空', self::MUST_VALIDATE, 'regex', self::MODEL_BOTH],
			['developer', 'require', '模块开发者不能为空', self::MUST_VALIDATE, 'regex', self::MODEL_BOTH],
			['version', 'require', '模块版本不能为空', self::MUST_VALIDATE, 'regex', self::MODEL_BOTH],
			['admin_menu', 'require', '模块菜单节点不能为空', self::MUST_VALIDATE, 'regex', self::MODEL_BOTH],
		];
	/**
	 * 自动完成规则
	 */
		...
		foreach ($module_list as &$val) {
			switch ($val['status']) {
				case '-2': // 损坏
					...
					$val['right_button']['damaged']['attribute'] = 'class="label label-danger ajax-get" href="'.U('setStatus', ['status' => 'delete', 'ids' => $val['id']]).'"';
					break;
				case '-1': // 未安装
					$val['status_icon']                                 = '<i class="fa fa-download text-success"></i>';
					$val['right_button']['install_before']['title']     = '安装';
					$val['right_button']['install_before']['attribute'] = 'class="label label-success" href="'.U('install_before', ['name' => $val['name']]).'"';
					break;
				case '0': // 禁用
					$val['status_icon']                                   = '<i class="fa fa-ban text-danger"></i>';
					$val['right_button']['update_info']['title']          = '更新菜单';
					$val['right_button']['update_info']['attribute']      = 'class="label label-info ajax-get no-refresh" href="'.U('updateInfo', ['id' => $val['id']]).'"';
					...
					$val['right_button']['uninstall_before']['attribute'] = 'class="label label-danger " href="'.U('uninstall_before', ['id' => $val['id']]).'"';
					break;
				case '1': // 正常
