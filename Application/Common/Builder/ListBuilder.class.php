    ...
    public function addTopButton($type, $attribute = null)
    {
        switch ($type) {
            ...
            case 'delete': // 添加删除按钮(我没有反操作，删除了就没有了，就真的找不回来了)
                ...
                $this->_top_button_list[] = $my_attribute;
                break;
            case 'delete2': // 添加删除按钮(我没有反操作，删除了就没有了，就真的找不回来了)
                // 预定义按钮属性以简化使用
                $my_attribute['title']       = '删除';
                $my_attribute['target-form'] = 'ids';
                $my_attribute['class']       = 'btn btn-danger-outline btn-pill ajax-post confirm';
                $my_attribute['model']       = $attribute['model'] ?: CONTROLLER_NAME;
                $my_attribute['href']        = U(
                    MODULE_NAME . '/' . CONTROLLER_NAME . '/updateStatus',
                    array(
                        'status' => 'delete',
                        'model'  => $my_attribute['model'],
                    )
    ...
    public function addSearchItem($name, $type, $title='', $tip='', $options = array(), $extra_class = '', $extra_attr = '')
    {
        ...
        if ($type == 'dateranger') {
            ...
            if ($dates) {
                ...
                $start_date = datetime("-{$options} days", 'Y-m-d');
    ...
    public function addRightButton($type, $attribute = null)
    {
        switch ($type) {
            ...
            case 'forbid': // 改变记录状态按钮，会更具数据当前的状态自动选择应该显示启用/禁用
                 ...
                 * 把'新增'踢走自己霸占title这个位置，其它的属性同样道理
                 */
                if (isset($attribute['forbid0']) && is_array($attribute['forbid0'])) {
                    $my_attribute['forbid0'] = array_merge($my_attribute['forbid0'], $attribute['forbid0']);
                }
                if (isset($attribute['forbid1']) && is_array($attribute['forbid1'])) {
             ...
             //同意加入
             case 'agree': // 改变记录状态按钮，会更具数据当前的状态自动选择应该显示启用/禁用
                //预定义按钮属
                $my_attribute['type']             = 'agree';
                $my_attribute['model']            = $attribute['model'] ?: CONTROLLER_NAME;
                $my_attribute['agree0']['name']  = 'agree';
                $my_attribute['agree0']['title'] = '同意加入';
                $my_attribute['agree0']['class'] = 'label label-success-outline label-pill ajax-get confirm';
                $my_attribute['agree0']['href']  = U(
                    MODULE_NAME . '/' . CONTROLLER_NAME . '/setStatus',
                    array(
                        'status' => 'agree',
                        'ids'    => '__data_id__',
                ...
                $my_attribute['agree1']['name']  = 'agree';
                $my_attribute['agree1']['title'] = '取消加入';
                $my_attribute['agree1']['class'] = 'label label-warning-outline label-pill ajax-get confirm';
                $my_attribute['agree1']['href']  = U(
                    MODULE_NAME . '/' . CONTROLLER_NAME . '/setStatus',
                    array(
                        'status' => 'disagree',
                        'ids'    => '__data_id__',
                 ...
                 * 把'新增'踢走自己霸占title这个位置，其它的属性同样道理
                 */
                if (isset($attribute['agree0']) && is_array($attribute['agree0'])) {
                    $my_attribute['agree0'] = array_merge($my_attribute['agree0'], $attribute['agree0']);
                }
                if (isset($attribute['agree1']) && is_array($attribute['agree1'])) {
                    $my_attribute['agree1'] = array_merge($my_attribute['agree1'], $attribute['agree1']);
                }
            ...
            case 'setTop': // 改变记录状态按钮，会更具数据当前的状态自动选择应该显示启用/禁用
                //预定义按钮属
                $my_attribute['type']             = 'forbids';
                $my_attribute['model']            = $attribute['model'] ?: CONTROLLER_NAME;
                $my_attribute['forbids0']['name']  = 'forbids';
                $my_attribute['forbids0']['title'] = '取消置顶';
                $my_attribute['forbids0']['class'] = 'label label-success-outline label-pill ajax-get confirm ajax-get';
                $my_attribute['forbids0']['href']  = U(
                    MODULE_NAME . '/' . CONTROLLER_NAME . '/setStatus',
                    array(
                        'status' => 'resumes',
                        'ids'    => '__data_id__',
                ...
                $my_attribute['forbids1']['name']  = 'forbids';
                $my_attribute['forbids1']['title'] = '置顶';
                $my_attribute['forbids1']['class'] = 'label label-warning-outline label-pill ajax-get confirm ajax-get';
                $my_attribute['forbids1']['href']  = U(
                    MODULE_NAME . '/' . CONTROLLER_NAME . '/setStatus',
                    array(
                        'status' => 'forbids',
                        'ids'    => '__data_id__',
                 ...
                 * 把'新增'踢走自己霸占title这个位置，其它的属性同样道理
                 */
                if (isset($attribute['forbids0']) && is_array($attribute['forbids0'])) {
                    $my_attribute['forbids0'] = array_merge($my_attribute['forbids0'], $attribute['forbids0']);
                }
                if (isset($attribute['forbids1']) && is_array($attribute['forbids1'])) {
                    $my_attribute['forbids1'] = array_merge($my_attribute['forbids1'], $attribute['forbids1']);
                }
                ...
                break;
                //已使用和未使用
            case 'is_used': // 改变记录状态按钮，会更具数据当前的状态自动选择应该显示启用/禁用
                //预定义按钮属
                $my_attribute['type']             = 'forbid3';
                $my_attribute['model']            = $attribute['model'] ?: CONTROLLER_NAME;
                $my_attribute['forbid30']['name']  = 'forbid3';
                $my_attribute['forbid30']['title'] = '已使用';
                $my_attribute['forbid30']['class'] = 'label label-success-outline label-pill ajax-get confirm';
                $my_attribute['forbid30']['href']  = U(
                    MODULE_NAME . '/' . CONTROLLER_NAME . '/setStatus',
                    array(
                        'status' => 'resume3',
                        'ids'    => '__data_id__',
                ...
                $my_attribute['forbid31']['name']  = 'forbid3';
                $my_attribute['forbid31']['title'] = '未使用';
                $my_attribute['forbid31']['class'] = 'label label-warning-outline label-pill ajax-get confirm';
                $my_attribute['forbid31']['href']  = U(
                    MODULE_NAME . '/' . CONTROLLER_NAME . '/setStatus',
                    array(
                        'status' => 'forbid3',
                        'ids'    => '__data_id__',
                 ...
                 * 把'新增'踢走自己霸占title这个位置，其它的属性同样道理
                 */
                if (isset($attribute['forbid30']) && is_array($attribute['forbid30'])) {
                    $my_attribute['forbid30'] = array_merge($my_attribute['forbid30'], $attribute['forbid30']);
                }
                if (isset($attribute['forbid31']) && is_array($attribute['forbid31'])) {
                    $my_attribute['forbid31'] = array_merge($my_attribute['forbid31'], $attribute['forbid31']);
                }
            ...
            case 'recycle':
                 ...
                 * 把'新增'踢走自己霸占title这个位置，其它的属性同样道理
                 */
                if (isset($attribute['recycle-1']) && is_array($attribute['recycle-1'])) {
                    $my_attribute['recycle-1'] = array_merge($my_attribute['recycle-1'], $attribute['recycle-1']);
                }
                if (isset($attribute['recycle1']) && is_array($attribute['recycle1'])) {
        ...
        foreach ($this->_table_data_list as &$data) {
            ...
            if ($this->_right_button_list) {
                foreach ($this->_right_button_list as $right_button) {
                    ...
                    if (isset($right_button['type']) && $right_button['type'] === 'agree') {
                        $right_button = $right_button['agree' . $data['agree_coming']];
                    }

                    if (isset($right_button['type']) && $right_button['type'] === 'forbids') {
                        $right_button = $right_button['forbids' . $data['status']];
                    }
                    if (isset($right_button['type']) && $right_button['type'] === 'forbid3') {
                        $right_button = $right_button['forbid3' . $data['status']];
                    }
            ...
            if ($this->_alter_data_list) {
                foreach ($this->_alter_data_list as $alter) {
                    if ($data[$alter['condition']['key']] === $alter['condition']['value']) {
                        if ($alter['alter_data']['right_button'] && is_array($alter['alter_data']['right_button'])) {
