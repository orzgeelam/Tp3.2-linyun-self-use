...
class Str {
     ...
     * @return str
     */
    static public function cutStr($str, $start, $length, $charset='utf-8', $suffix = true) {
        $str = trim($str);
        $length = $length * 2;
        if($length){
            //截断字符
     ...
     */
    static public function parseAttr($value, $type = null) {
        switch ($type) {
            default:
     ...
     * 可根据自身的业务需求更改
     */
    static public function createOutTradeNo() {
        $year_code = array('A', 'B', 'C', 'D', 'E', 'F', 'G', 'H', 'I', 'J');
        return 'OC'.$year_code[intval(date('Y')) - 2010] .strtoupper(dechex(date('m'))) .
     ...
     * @param  string $text
     */
    static public function str2url($text) {
