...
function slog($log, $type = 'log', $css = '')
{
	if (is_string($type)) {
		...
		if (method_exists('Slog', $type) || in_array($type, Slog::$log_types)) {
			return call_user_func(['Slog', $type], $log, $css);
		}
	}
...
class Slog
{
	...
	public static function error_handler($errno, $errstr, $errfile, $errline)
	{
		...
		if (function_exists('ptrace')) {
			if (false === stripos($errstr, 'Only variables should be passed by reference ') && false === stripos($errstr, 'should be compatible') && (false === stripos($errstr, 'Array to string conversion')) && false === stripos($errstr, '无法加载控制器:Undefined')) {
				ptrace($msg);
			}
		}
		$level     = 'ERR';
		$tmp_level = str_ireplace('E_', '', $severity);
		$label     = '';
		switch ($tmp_level) {
			case 'WARNING':
			case 'USER_WARNING':
	...
	protected static function sendToClient($tabid, $client_id, $logs, $force_client_id)
	{
		...
		if ($msg == false) {
			$logs['logs'] = var_export($logs['logs'], true);
			$msg          = json_encode($logs);
		}
