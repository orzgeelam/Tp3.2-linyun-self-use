...
function import($class, $baseUrl = '', $ext = EXT)
{
	...
	if (empty($baseUrl)) {
		...
		} else if ('Common' == $class_strut[0]) {
			//加载公共模块的类库
			$baseUrl = COMMON_PATH;
			$class   = substr($class, 7);
		} else if (in_array($class_strut[0], ['Think', 'Org', 'Behavior', 'Com', 'Vendor']) || is_dir(LIB_PATH.$class_strut[0])) {
			// 系统类库包和第三方类库包
			$baseUrl = LIB_PATH;
		} else {
			// 加载其他模块的类库
