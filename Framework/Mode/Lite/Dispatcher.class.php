...
class Dispatcher
{
		...
		if (C('APP_SUB_DOMAIN_DEPLOY')) {
			...
			if (isset($rules[$_SERVER['HTTP_HOST']])) {
				...
				if (!empty($domain)) {
					...
					} else if (isset($rules['*.'.$domain2]) && !empty($domain3)) {
						// 泛三级域名
						$rule      = $rules['*.'.$domain2];
						$panDomain = $domain3;
					} else if (isset($rules['*']) && !empty($domain2) && 'www' != $domain2) {
						// 泛二级域名
						$rule      = $rules['*'];
						$panDomain = $domain2;
					}
	...
	private static function getModule($var)
	{
		...
		if ($maps = C('URL_MODULE_MAP')) {
			...
			} else if (array_search(strtolower($module), $maps)) {
				// 禁止访问原始模块
				return '';
			}
		}
